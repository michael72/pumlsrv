...
- `pumlsrv-server` — a headless launcher, handy inside containers. It runs `pumlsrv -u -n -N "${PUMLSRV_PORT:-8080}"` (no update check, no stored settings, no browser) with its output discarded. The port comes from `$PUMLSRV_PORT`, defaulting to `8080`.
